    ...
    <Listbox value={config.current} onChange={(option) => handleFormUpdate(option)}>
      {({ open }) => (
        <div className={ `flex flex-col items-end ${props.config.id}` }>
          <Listbox.Label className={ `block text-sm font-medium text-slate-500 ${props.labelClassname}` }>
            { props.config.label }
          </Listbox.Label>
          <div className={ `mt-1 relative ${props.wrapperClassname}` }>
            <Listbox.Button 
              className={ `relative w-full bg-slate-900 border border-slate-300 rounded-md shadow-sm pl-3 pr-10 py-0.5 text-left 
            ...
            >
              <Listbox.Options 
                className="absolute z-10 mt-1 w-full bg-slate-800 shadow-lg max-h-60 rounded-md py-1 text-base 
                ...
                {props.config.selectOptions.map((option,idx) => (
                  <Listbox.Option
                    ...
                    className={`cursor-default select-none relative py-2 pl-8 pr-4 `+
                      isActive(config, option) ? 'text-white bg-cyan-600' : 'text-slate-50'
                    }
                    value={option}
                  >
                    {({ selected, active }) => (
