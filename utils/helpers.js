module.exports = {
  formatSweeperTimestamps : (config) => {
    let start = new Date();
    let temp = new Date(start);
    let nextSweep = new Date(temp.setHours(temp.getHours() + config.frequency ));
    nextSweep.setMinutes( Math.round(nextSweep.getMinutes() / 5) * 5 );

    temp = new Date(start);
    let expiration = new Date(temp.setHours(temp.getHours() + config.duration ));
  ...
  getRemainingTime: (expiration, created = null) => {
    ...
    if (remaining < msPerMinute) return `${expiration_date} | ${expiration_time.replace(' ','').toLowerCase()} | ${Math.round(remaining/1000)}s`;  
    else if (remaining < msPerHour) return `${expiration_date} | ${expiration_time.replace(' ','').toLowerCase()} | ${Math.round(remaining/msPerMinute)}m`;
    ...
    else if(remaining < msPerDay * 7) return `${expiration_date} | ${expiration_time.replace(' ','').toLowerCase()} | ${Math.round(remaining/msPerDay)}d`;
    else if(remaining < msPerDay * 28) return `${expiration_date} | ${expiration_time.replace(' ','').toLowerCase()} | ${Math.round(remaining/(msPerDay*7))}W`;
    else return `${expiration_date} | ${expiration_time.replace(' ','').toLowerCase()} | ${Math.floor(remaining/(msPerDay*28))}M`;
  },

  getElapsedTime: (release_date) => {
    let msPerMinute = 60 * 1000;
    let msPerHour = msPerMinute * 60;
    let msPerDay = msPerHour * 24;
